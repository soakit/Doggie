...
import { CountDownText } from '../../components/CountDown/index'
...
class MakeScreen extends React.Component {
			...
			>
				<View style={styles.creattionTopStyles}>
					...
					{this.state.previewVideo && this.state.videoUploaded ? (
						<TouchableOpacity
							style={styles.creattionTopRightStyles}
				...
				{this.state.previewVideo ? (
					<View style={{ width: width, height: 300 }}>
						...
						>
							...
							this.state.record ? (
								<TouchableOpacity
									style={{ position: 'absolute', right: 20, top: 239 }}
								...
								<View>
									<View style={styles.progressBox}>
										<View
											style={[
												styles.progressBar,
												{ width: width * this.state.videoUploadProgress }
											]}
										/>
									</View>
									<Text>
										正在生成静音视频，已完成
										{(this.state.videoUploadProgress * 100).toFixed(2) + '%'}
									</Text>
								</View>
							) : null}
	...
	getQiniuToken = () => {
		...
		var signatureURL = conf.base + conf.signature
		return request.post(signatureURL, {
			accessToken: accessToken || 'b006094b-fd1c-4ccb-9700-3471272352dd',
			cloud: 'qiniu',
			type: 'video'
		})
	}
	...
	//发送异步请求
	_upload(body) {
		console.log('upload body:', body, url)
		...
		xhr.onload = () => {
			...
			if (response) {
				...
				//上传视频到服务器进行同步
				var accessToken = this.state.user.accessToken
				var videoUrl = conf.base + conf.video
				request
					.post(videoUrl, {
